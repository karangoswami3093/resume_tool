  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <svg width="52" height="52" viewBox="0 0 52 52">
        <circle cx="26" cy="26" r={r} fill="none" stroke="currentColor" className="text-slate-200" strokeWidth="4" />
        <motion.circle
          cx="26" cy="26" r={r}
        ...
        />
        <text x="26" y="30" textAnchor="middle" fill="currentColor" className="text-slate-700" fontSize="10" fontWeight="600">{value}</text>
      </svg>
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-slate-400 text-sm animate-pulse">Calculating ATS score...</div>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-4 p-4 overflow-y-auto h-full">
      ...
      <div className="flex flex-col items-center gap-2 py-4">
        <div className="relative">
          <svg width="120" height="120" viewBox="0 0 120 120">
            <circle cx="60" cy="60" r="45" fill="none" stroke="currentColor" className="text-slate-200" strokeWidth="8" />
            <motion.circle
              cx="60" cy="60" r="45"
      ...
      <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
        <div className="flex justify-between items-center mb-2">
          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Keyword Match</span>
      ...
      {score.matchedKeywords.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <CheckCircle2 className="w-3.5 h-3.5 text-[#3A7A62]" />
            Matched ({score.matchedKeywords.length})
          </p>
          <div className="flex flex-wrap gap-1.5">
            {score.matchedKeywords.slice(0, 12).map((kw, i) => (
              <span key={i} className="px-2 py-0.5 bg-[#3A7A62]/10 text-[#3A7A62] rounded-full text-[10px] font-medium border border-[#3A7A62]/20">
                {kw}
              </span>
            ))}
          </div>
      ...
      {score.missingKeywords.length > 0 && (
        <div>
          <button
            ...
            className="w-full flex items-center justify-between text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2"
          >
      ...
      {score.suggestions.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Lightbulb className="w-3.5 h-3.5 text-[#7ECBC4]" />
            Suggestions
          </p>
          <div className="space-y-1.5">
            {score.suggestions.map((s, i) => (
              <div key={i} className="flex gap-2 bg-[#1E5C40]/10 border border-[#7ECBC4]/20 rounded-lg p-2.5">
                <span className="text-[#7ECBC4] text-xs mt-0.5">•</span>
      ...
      {score.strengths.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <TrendingUp className="w-3.5 h-3.5 text-[#7ECBC4]" />
            Strengths
          </p>
          <div className="space-y-1.5">
            {score.strengths.map((s, i) => (
              <div key={i} className="flex gap-2 bg-[#1E5C40]/10 rounded-lg p-2.5 border border-[#7ECBC4]/20">
                <span className="text-[#7ECBC4] text-xs mt-0.5">✓</span>
                <p className="text-xs text-slate-600 leading-relaxed">{s}</p>
              </div>
            ))}
      ...
      {jdAnalysis && (
        <div className="bg-slate-50 rounded-xl p-3 border border-slate-200">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Target Role</p>
          <p className="text-sm font-semibold text-slate-900">{jdAnalysis.jobTitle}</p>
          <p className="text-xs text-slate-400">{jdAnalysis.industry} · {jdAnalysis.experienceLevel}</p>
        </div>
      )}
    </div>
  );
