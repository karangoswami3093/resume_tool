...
export default function ResumePreview({ resumeData, isOptimizing, pageCount = "1" }: ResumePreviewProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Preview header */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-200 dark:border-[#AED6CF]/25 bg-white dark:bg-transparent shrink-0">
        <div className="flex items-center gap-2">
          <h2 className="text-xs font-semibold text-slate-500 dark:text-white/50 uppercase tracking-widest">
            Resume Preview
          </h2>
          {resumeData && (
            <span className="text-[10px] text-slate-400 dark:text-white/30 bg-slate-100 dark:bg-[#AED6CF]/20 px-2 py-0.5 rounded-full">
        ...
        {resumeData && (
          <motion.button
            ...
            className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-[#647FBC] hover:bg-[#91ADC8] text-[#1e2a5e] text-xs font-semibold transition-colors shadow-md shadow-[#647FBC]/25"
          >
      ...
      <div
        ref={containerRef}
        className="flex-1 overflow-auto bg-slate-200 dark:bg-[#1e2a5e] p-6 flex justify-center resume-scroll"
      >
        {isOptimizing ? (
          <div className="flex flex-col items-center justify-center h-full gap-4">
            <div className="relative">
              <Loader2 className="w-10 h-10 animate-spin text-[#91ADC8]" />
            </div>
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3 select-none">
            <div className="w-20 h-28 border-2 border-dashed border-slate-300 dark:border-[#AED6CF]/30 rounded-lg flex items-center justify-center">
